...
angular
  ...
  .controller("SummernoteController", [
    ...
    function($scope, $attrs, $timeout) {
      ...
      if (
        (angular.isDefined($attrs.height) &&
          (summernoteConfig.height = +$attrs.height),
        angular.isDefined($attrs.minHeight) &&
          (summernoteConfig.minHeight = +$attrs.minHeight),
        angular.isDefined($attrs.maxHeight) &&
      ...
      (callbacks.onInit = $scope.init),
        (callbacks.onEnter = function(evt) {
          $scope.enter({ evt: evt });
        }),
        (callbacks.onFocus = function(evt) {
          $scope.focus({ evt: evt });
        }),
        (callbacks.onPaste = function(evt) {
          $scope.paste({ evt: evt });
        }),
        (callbacks.onKeyup = function(evt) {
          $scope.keyup({ evt: evt });
        }),
        (callbacks.onKeydown = function(evt) {
          $scope.keydown({ evt: evt });
        }),
        angular.isDefined($attrs.onImageUpload) &&
          (callbacks.onImageUpload = function(files) {
            $scope.imageUpload({ files: files, editable: $scope.editable });
          }),
        angular.isDefined($attrs.onMediaDelete) &&
        ...
        (this.activate = function(scope, element, ngModel) {
          var updateNgModel = function() {
            ...
            element.summernote("isEmpty") && (newValue = ""),
              ngModel &&
          ...
          editor$.find(".note-toolbar").click(function() {
            updateNgModel(),
              editor$.hasClass("codeview")
                ? (editor$.on("keyup", updateNgModel),
                  ngModel &&
                    (unwatchNgModel = scope.$watch(
                      function() {
                ...
                : (editor$.off("keyup", updateNgModel),
                  angular.isFunction(unwatchNgModel) && unwatchNgModel());
          }),
            ngModel &&
            ...
            angular.isDefined($attrs.editable) &&
              ($scope.editable = editor$.find(".note-editable")),
            angular.isDefined($attrs.editor) && ($scope.editor = element),
